    ...
    //changer couleur des icns quand sélectionnés
      document.getElementById('form-msg').onchange = function (e) {
        ...
        if(input.checked) {
          document.querySelector(`.mood-labels[for="${input.id}"]`).style.color= 'teal'
      } else {
        document.querySelector(`.mood-labels[for="${input.id}"]`).style.color= 'indianred'
      }
      })
    }
    ...
    document.getElementById('form-msg').onsubmit = function(e){
      ...
      if (moodSelected) {
        socket.emit('chat message', document.getElementById('m').value, moodSelected);
      } else {
        socket.emit('chat message', document.getElementById('m').value);
      }
      document.getElementById('m').value = ''
      document.getElementById('m').style.cssText = 'height: fit-content';
      document.querySelectorAll('.mood-labels').forEach((label)=>
  ...
  const msgPlaceholder = createElement('div', document.getElementById('messages').lastElementChild.querySelector('.message-by-date'),'');
  const newMsg = createElement('div', msgPlaceholder,['message'], "", "", msg._id);
  const dateMsg = createElement('p', newMsg,['message-date'], moment(msg.send_date).format('hh:mm'), "");
  const textMsg = createElement('span', newMsg,['message-text'], msg.text, "", msg._id);
  if (msg.mood) {const moodMsg = addMoodElement(newMsg, msg.mood) }
  const actionsMsg= createElement('div', newMsg,['message-actions']);
  const repondre = createElement('p', actionsMsg,['repondre'], 'Répondre' );
  const reponses = createElement('p', actionsMsg,['reponses'], msg.response.length ?`${msg.response.length} Réponses`:'');
    ...
    socket.on('post answer', function(ans, msg_id){
      ...
      const newAns = createElement('div', AnsPlaceholder,['answer'], ans)
        if (!AnsPlaceholder.hasChildNodes()) {
          ...
          const formerNb = document.querySelector(`.message[data-msg-id="${msg_id}"]`).querySelector(`.message-actions`).querySelector(`.reponses`).innerText.split(' ')[0];
...
// Afficher l'input réponse
    repondreButtons.forEach((button)=>{
      button.onclick = (e)=>{
      const answersForm = e.target.parentElement.parentElement.nextSibling.nextSibling
      answersForm.classList.toggle('hidden')
      const position  = answersForm.offsetTop/2
      if (!answersForm.classList.contains('hidden')) document.getElementById('messages').scrollTo(0, position);
...
// Afficher les réponses
    reponsesButtons.forEach((button)=>{
      button.onclick = (e)=>{
      const answersPlaceholder = e.target.parentElement.parentElement.parentElement.lastElementChild;
      answersPlaceholder.classList.toggle('hidden')
      }
    }
    )
